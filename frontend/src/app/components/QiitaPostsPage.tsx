...
function QiitaPostsPage({ allPosts, limit }: Props) {
  const [currentPage, setCurrentPage] = useState(1);
  const totalPages = Math.ceil(allPosts.length / limit);

  const pageItems = useMemo(
    () => allPosts.slice((currentPage - 1) * limit, currentPage * limit),
        ...
        {pageItems.map((posts: Posts) => {
          return (
            <Link href={posts.url} key={posts.id} className="card bg-base-100 w-96 shadow-lg">
              <figure>
                <Image src={posts.ogpImageUrl || thumbnail} alt="画像" width={400} height={300} priority={true} />
              </figure>
